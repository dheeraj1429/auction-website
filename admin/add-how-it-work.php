<?php 
    ...
    $pageName="Add/ Edit How it Works";
...
if(isset($_GET['id'])){
    $id = mysqli_real_escape_string($conn,ak_secure_string($_GET['id']));
    $dataQHit = mysqli_query($conn,"SELECT `id`, `type`, `title`, `img`, `desc`, `date_time`, `status` FROM `".$tblPrefix."cms_pages` WHERE type = 2 AND id = ".$_GET['id']);
